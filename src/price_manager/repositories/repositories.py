# %%writefile price_manager/repositories/repositories.py
...
class RepositorioStock(IRepositorioStock):
    """Repositorio en memoria para stock."""
...
class RepositorioCotizacionDolar(IRepositorioCotizacionDolar):
    """Repositorio en memoria para cotizaciones del dolar."""
    ...
    def crear(self, cotizacion: CotizacionDolar) -> CotizacionDolar:
        ...
        cotizacion_existente = self.leer_por_tipo_y_fecha(
            cotizacion.tipo,
            cotizacion.fecha,
        )
    ...
    def leer_historico_por_tipo(self, tipo: str) -> list[CotizacionDolar]:
        ...
        resultado = [
            item
            for item in self._data.values()
            if item.tipo == tipo_normalizado
        ]

        return sorted(resultado, key=lambda item: item.fecha)
