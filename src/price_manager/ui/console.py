...
class PriceManagerConsole:
  ...
  def _verificar_cotizacion_inicial(self) -> None:
    ...
    hoy = date.today()
    cotizacion_hoy = self._servicio_cotizacion.buscar_por_tipo_y_fecha("Oficial", hoy)
    ...
    try:
      cotizacion = self._servicio_cotizacion.actualizar_cotizacion_tiempo_real("Oficial")
      print(
  ...
  def _listar_productos(self, productos: list | None = None) -> None:
    productos = productos if productos is not None else self._servicio_producto.listar_todos()
    if not productos:
    ...
    if cotizacion_ref is not None:
      print(f"Cotizacion de referencia (OFICIAL): {cotizacion_ref.valor:.2f} ARS/USD")
    else:
      print("Cotizacion de referencia (OFICIAL): no disponible")

    for producto in productos:
      precio_line = self._formatear_precio_dual(producto.precio.valor, producto.precio.moneda, cotizacion_ref.valor if cotizacion_ref else None)
      print(
        f"ID: {producto.id} | {producto.nombre} | {precio_line}"
      )
  ...
  def _seleccionar_producto(self) -> object | None:
    ...
    while True:
      ...
      if opcion == "1":
        ...
        producto_id = self._read_int("ID producto (o 'b' para volver): ", allow_back=True)
      ...
      if opcion == "2":
        ...
        if not filtrados:
          ...
          cercanos = get_close_matches(termino, [n.lower() for n in nombres], n=5, cutoff=0.45)
          if cercanos:
            sugeridos = [p for p in productos if p.nombre.lower() in cercanos]
            ...
            if len(sugeridos) == 1:
              ...
              for idx, prod in enumerate(sugeridos, start=1):
                print(f"{idx}. {prod.nombre} (ID {prod.id}, {prod.precio.valor:.2f} {prod.precio.moneda})")
              indice = self._read_int("Elegi indice (o 'b' para volver): ", allow_back=True)
              if indice is None:
                continue
        ...
        if len(filtrados) == 1:
          unico = filtrados[0]
          print(f"Se selecciono automaticamente: {unico.nombre} (ID {unico.id})")
          return unico
  ...
  def _mostrar_detalle_producto(self, producto) -> None:
    ...
    print(f"Precio: {self._formatear_precio_dual(producto.precio.valor, producto.precio.moneda, cotizacion_ref.valor if cotizacion_ref else None)}")
  ...
  def _actualizar_cotizacion_tiempo_real(self) -> None:
    tipo = input("Tipo (Oficial/Blue/Bolsa/CCL/Cripto/Tarjeta/Mayorista) o 'b' para volver: ").strip()
    if tipo.lower() == "b":
      return
    tipo = tipo or "Oficial"
    cotizacion_hoy = self._servicio_cotizacion.buscar_por_tipo_y_fecha(tipo, date.today())
    if cotizacion_hoy is not None:
      print(
  ...
  def _seleccionar_o_crear_categoria(self) -> Categoria | None:
    while True:
      ...
      if opcion == "1":
        ...
        indice = self._read_int("Indice categoria (o 'b' para volver): ", allow_back=True)
        if indice is None:
          continue
  ...
  def _seleccionar_o_crear_proveedor(self) -> Proveedor | None:
    while True:
      ...
      if opcion == "1":
        ...
        indice = self._read_int("Indice proveedor (o 'b' para volver): ", allow_back=True)
        if indice is None:
          continue
  ...
  def _crear_producto(self) -> None:
    ...
    next_price_id = max((p.id for p in self._servicio_precio.listar_todos()), default=0) + 1
    precio = self._servicio_precio.crear(Precio(id=next_price_id, valor=valor, moneda=moneda))
    next_product_id = max((p.id for p in self._servicio_producto.listar_todos()), default=0) + 1
    producto = Producto(
      id=next_product_id,
      nombre=nombre,
    ...
    print(f"Producto creado: ID {producto.id} | {producto.nombre} | {precio.valor:.2f} {precio.moneda}")
  ...
  def _comparar_competencia_producto(self, producto) -> None:
    ...
    try:
      ...
      print("Recomendacion: reintentar mas tarde o verificar conectividad de red.")
      return

    print("\nComparacion web (books.toscrape.com)")
    print(
      "Producto local: "
    ...
    print(f"Precio competencia: GBP {result['competitor_price_gbp']:.2f}")
    print(f"Diferencia numerica: {result['difference']:.2f} ({result['difference_pct']:.2f}%)")
    print(f"URL: {result['competitor_url']}")
    print("Nota: la comparacion es referencial, sin conversion automatica de moneda.")

  def _formatear_precio_dual(self, valor: float, moneda: str, cotizacion_ars_usd: float | None) -> str:
  ...
  def _eliminar_producto_objetivo(self, producto) -> bool:
    confirmar = input(f"Confirmar eliminacion de '{producto.nombre}'? [s/n]: ").strip().lower()
    if confirmar != "s":
  ...
  def _eliminar_categoria(self) -> None:
    ...
    if categoria is None:
      return
    en_uso = [p for p in self._servicio_producto.listar_todos() if p.categoria.id == categoria.id]
    if en_uso:
      ...
      return
    confirmar = input(f"Confirmar eliminacion de categoria '{categoria.nombre}'? [s/n]: ").strip().lower()
    if confirmar != "s":
      print("Eliminacion cancelada.")
      return
  ...
  def _eliminar_proveedor(self) -> None:
    ...
    if proveedor is None:
      return
    en_uso = [p for p in self._servicio_producto.listar_todos() if p.proveedor.id == proveedor.id]
    if en_uso:
      ...
      for prod in en_uso:
        print(f"- {prod.nombre} (ID {prod.id})")
      return
    confirmar = input(f"Confirmar eliminacion de proveedor '{proveedor.nombre_legal}'? [s/n]: ").strip().lower()
    if confirmar != "s":
      print("Eliminacion cancelada.")
      return
  ...
  def _listar_cotizaciones(self) -> None:
    ...
    for cot in sorted(cotizaciones, key=lambda x: (x.fecha, x.tipo, x.id)):
      print(f"ID {cot.id} | {cot.tipo} | {cot.fecha.isoformat()} | {cot.valor:.2f}")

  def _actualizar_cotizacion_manual(self) -> None:
    cot_id = self._read_int("ID cotizacion a actualizar (o 'b' para volver): ", allow_back=True)
    if cot_id is None:
      return
    ...
    if cot is None:
      ...
      return
    nuevo_valor = self._read_float("Nuevo valor (o 'b' para volver): ", allow_back=True)
  ...
  def _eliminar_cotizacion(self) -> None:
    cot_id = self._read_int("ID cotizacion a eliminar (o 'b' para volver): ", allow_back=True)
    if cot_id is None:
      return
    cot = self._servicio_cotizacion.buscar_por_id(cot_id)
    if cot is None:
